...
from myapp.app import hopar2, hopar, get_max_items, get_full_name, get_car_make, get_car_wheels, get_roll_call, close_car
from myapp.app import Major
...
class MyTests(TestCase):
    ...
    @mock.patch('myapp.app.hopar2')
    def test_function1(self, mock_hopar2):
        mock_hopar2.return_value = 'AABB'
        self.assertEquals(hopar(),"aaaAABB")
        mock_hopar2.assert_called()
        #mock_hopar2.assert_called_once_with('baz')
        
    @mock.patch('myapp.app.Major.get_b')
    def test_function1(self, mock_get_b):
        mock_get_b.return_value = '1'
        m = Major(1,2)
        self.assertEquals(m.get_a(1),2)
        mock_get_b.assert_called()
